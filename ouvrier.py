...
class Ouvrier:
    def __init__(self, nom: str):
        self.__nom = nom
        self.__cout = 0
        self.lesRessources = {Ressource("pierre"): 0, Ressource("bois"): 0, Ressource("architecture"): 0,
    ...
    def ajouterRessource(self, ressource: Ressource, quantite: int) -> None:
        for key in self.lesRessources.keys():
            if ressource == key:
                self.lesRessources[key] += quantite

    def quantiteByRessource(self, uneRessource: Ressource) -> int:
        for ressource in self.lesRessources.keys():
